...
function initWidgetGenerator() {
    ...
    let currentType = 'standard'; // 'standard' | 'badge' | 'scorecard'
    ...
    function buildHtmlSnippet(type, theme, isPreview = false) {
    ...
    function buildIframeSnippet(type, theme) {
        ...
        const width = type === 'standard' ? '190' : (type === 'badge' ? '340' : '360');
        const height = type === 'standard' ? '190' : (type === 'badge' ? '98' : '290');
    ...
    function updateGenerator() {
        ...
        // Generate HTML for Preview (uses local paths so it loads in localhost)
        const previewHtml = buildHtmlSnippet(currentType, currentTheme, true);
        previewArea.innerHTML = `<span class="widget-preview-label">Live-Vorschau</span>` + previewHtml;

        // Generate Code for Textarea (uses absolute https://www.escapearchive.de paths for external sites)
        let finalCode = '';
        if (currentMode === 'html') {
            finalCode = buildHtmlSnippet(currentType, currentTheme, false);
        } else {
